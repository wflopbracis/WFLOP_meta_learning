...
def calculate_metric(file_path):
    ...
    for _ in range(n_folds):
        ...
        param_grid = {
            "estimator__n_estimators": [800, 825, 850, 900, 950, 1000],
            "estimator__max_depth": [None, 2, 4, 10, 20, 30],  
            "estimator__min_samples_split": [2, 6, 7, 8],  
            "estimator__min_samples_leaf": [1, 2, 3, 4, 5]
        }

        base_model = MultiOutputRegressor(RandomForestRegressor(random_state=0))
        
        grid_search = GridSearchCV(base_model, param_grid=param_grid, cv=5, scoring='neg_mean_absolute_error', verbose=1, n_jobs=-1)
